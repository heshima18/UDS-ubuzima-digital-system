...
(async function () {
            ...
            for (const district of province.districts) {
                for (const sector of district.sectors) {
                    for (const cell of sector.cells) {
                        l.push({province: province.name, name: cell.name, district: district.name, sector: sector.name, id: cell.id})
                    }
                }
            }
        }
        f = document.querySelector('form#add-cell-form')
    ...
    f.addEventListener('submit', async e =>{
        ...
        if(v){
            ...
            for (const input of i) {
             ...
             a = await request('add-cell',postschema);
    ...
    $(document).ready(function () {
            ...
            e = table.DataTable({
                        ...
                        render: function (e, t, a, n) {
                            return (
                                `<div class="d-inline-block text-nowrap">
                                    <button class="btn btn-sm btn-icon border border-3" data-bs-toggle="modal" data-bs-target="#view-cell" data-id="${a.id}"><i class="bx bx-show-alt"></i></button>
                                </div>`
                            );
                        },
                    },
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        attr: { "data-bs-toggle": "modal", "data-bs-target": "#add-cell" },
                    },
                ],
        ...
        table.find("tbody").on("click", ".delete-cell", function () {
            if (confirm("Are you sure you want to delete this cell?")) {
                e.row($(this).parents("tr")).remove().draw();
            }
        })
        $('#add-cell').on('shown.bs.modal', function () {
            const $modal = $(this);
            z = []
            for (let province of m) {
                province = province.provinces[0]
            ...
            let sector = f.querySelector('input#sector')
            sector.addEventListener('focus', function (e) {
                showRecs(this,z,'sector')
            })
        });
    });
